...
from utils import read_jsonl, write_jsonl, ensure_dir_exists
...
def get_prompt(tokenizer, question, answer, prompt_type):
...
def infer_feedback(args):
    ...
    tock = time()
    print(f'load model cost {tock-tick:.5f} secend')

    data = read_jsonl(args.infer_res_file)
    ...
    write_jsonl(output_data, args.output_file)
